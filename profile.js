...
async function save() {
  try {
    ...
    const setStatus = (txt) => {
      if (exportNote) exportNote.textContent = txt;
      if (topSaveStatus) topSaveStatus.textContent = txt;
    };
    const user = auth.currentUser;
    if (user) {
      setStatus("Saving...");
      await setDoc(doc(db, "profiles", user.uid), { ...state, updatedAt: new Date() });
      setStatus("Synced.");
    } else {
      setStatus("Saved.");
    }
...
async function loadCloud(uid) {
  try {
    ...
    if (docSnap.exists()) {
      ...
      // Simple merge or overwrite
      if (confirm("Found cloud save! Load it? (This replaces local data)")) {
        state = { ...defaults(), ...cloudData };
        hydrateFields();
        renderCards();
        updateAll();
      }
      ...
      <div class="cardItem" data-kind="${kind}" data-idx="${idx}">
        ...
        <label class="field">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
            ...
            ${kind === 'projects' ? `<button type="button" class="miniBtn aiBtn" data-idx="${idx}" data-action="deep-dive">✨ AI Deep Dive</button>` : ''}
          </div>
...
async function handleDeepDive(idx) {
  ...
  if (!p.name) return alert("Please name your project first.");

  const btn = document.querySelector(`button[data-action='deep-dive'][data-idx='${idx}']`);
  ...
  try {
    ...
    const res = await fetch("http://localhost:3001/api/interview", {
      ...
      body: JSON.stringify({ role: state.targetRole, context: p.notes || p.name || "A new project", type: "question" })
    });
    const data = await res.json();

    if (data.error) {
      alert("AI Error: " + data.error);
      if (btn) btn.textContent = "✨ AI Deep Dive";
      return;
    }

    if (!data.question) {
      alert("AI did not provide a question. Try adding more detail to your notes.");
      if (btn) btn.textContent = "✨ AI Deep Dive";
      return;
    }

    // 2. User answers
    const answer = prompt(`Refining '${p.name}':\n\nAI Recruiter says: "${data.question}"\n\nYour Answer (be specific with numbers):`);
    ...
    const res2 = await fetch("http://localhost:3001/api/interview", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ role: state.targetRole, original: p.notes, answer, type: "refine" })
    });
    ...
    console.error(e);
    alert("AI Server not running! Run 'node server/server.js' in terminal.");
  } finally {
    if (btn) btn.textContent = "✨ AI Deep Dive";
  }
}

// Updated handler attachment
function attachCardHandlers() {
  const onWrapClick = (wrap, kind) => {
    wrap?.addEventListener("click", (e) => {
      // Remove
      const rmBtn = e.target?.closest?.("button[data-action='remove']");
      if (rmBtn) {
      ...
      // Deep Dive
      const ddBtn = e.target?.closest?.("button[data-action='deep-dive']");
      if (ddBtn) {
        const idx = Number(ddBtn.getAttribute("data-idx"));
        handleDeepDive(idx);
      }
    });
  };
...
const statsWrap = $("#viewStats");
const countEl = $("#viewCount");

shareBtn?.addEventListener("click", async () => {
  const user = auth.currentUser;
  if (!user) return alert("Please Login first to create a Smart Link!");

  // In a real app, this would be your production URL
  const publicUrl = `http://localhost:3001/u/${user.uid}`;

  try {
    await navigator.clipboard.writeText(publicUrl);
    alert("Smart Link copied to clipboard!\nSend this to recruiters to track views.");
...
async function updateStats(uid) {
  if (!uid) return;
  try {
    const res = await fetch(`http://localhost:3001/api/stats/${uid}`);
    const data = await res.json();
...
function updateAll() {
  ...
  if (completionChip) completionChip.textContent = `${pct}% complete`;
  save();
}
